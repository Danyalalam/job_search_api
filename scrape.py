...
class LinkedInJobScraper:
    ...
    def _extract_salary(self, soup):
        ...
        try:
            ...
            salary_text = soup.find(string=re.compile(r'\$?\d+,?\d*\s*(-|to)\s*\$?\d+,?\d*'))
            return salary_text.strip() if salary_text else "Not specified"
        except Exception:
            return "Not specified"
...
def main():
    # Example usage
    scraper = LinkedInJobScraper(title="AI Engineer", location="Toronto")
